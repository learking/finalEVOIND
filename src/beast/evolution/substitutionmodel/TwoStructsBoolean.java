...
public class TwoStructsBoolean extends SubstitutionModel.Base {
	
    public Input<BooleanParameter> indicatorInput = new Input<BooleanParameter>("indicator", "whether the first structure is preferred", Validate.REQUIRED);
    ...
    double[] getCodonProb(){
    	final Boolean preferFirst = indicatorInput.get().getValue();
    	double[] codonProbResult = new double[codonNr];
    	if(preferFirst){
    		for (int i = 0; i < codonNr; i++) {
    			codonProbResult[i] = codonProb_struct1[i];
    		}
    	}else{
    		for (int i = 0; i < codonNr; i++) {
    			codonProbResult[i] = codonProb_struct2[i];
    		}
    	}
