...
public class TreeLikelihoodV extends GenericTreeLikelihood {
    ...
    public Input<Scaling> scaling = new Input<TreeLikelihoodV.Scaling>("scaling", "type of scaling to use, one of " + Arrays.toString(Scaling.values()) + ". If not specified, the -beagle_scaling flag is used.", Scaling._default, Scaling.values());
	...
	*/
    ...
    public void initAndValidate() throws Exception {
		...
		*/
        ...
        System.out.println("lear: stateCount" + nStateCount);
        System.out.println("lear: patternCount" + nPatterns);
        
        if (nStateCount == 4) {
            likelihoodCore = new BeerLikelihoodCore4();
        } else {
            likelihoodCore = new BeerLikelihoodCore(nStateCount);
            System.out.println("lear: use BeerLikelihoodCore(" + nStateCount +")");
        }
        System.out.println("TreeLikelihood uses " + likelihoodCore.getClass().getName());

        proportionInvariant = m_siteModel.getProportionInvariant();
        System.out.println("lear: proportionInvariant" + proportionInvariant);
        
        m_siteModel.setPropInvariantIsCategory(false);
        if (proportionInvariant > 0) {
            System.out.println("lear: calcConstantPatternIndices is used");        	
    ...
    void initCore() {
        ...
        if (m_useAmbiguities.get()) {
            System.out.println("lear: setPartials is used");        	
            setPartials(treeInput.get().getRoot(), dataInput.get().getPatternCount());
        } else {
            System.out.println("lear: setStates is used");        	
    ...
    public double calculateLogP() throws Exception {
        ...
        */
        ...
        if (logP > 0 || (likelihoodCore.getUseScaling() && m_nScale > X)) {
            System.out.println("lear: logP greater than 0");        	
