...
public class TwoStructs extends GeneralSubstitutionModel {
    public Input<RealParameter> percentInput = new Input<RealParameter>("percent", "kappa parameter in TwoStructs model", Validate.REQUIRED);
	
    public Input<RealParameter> kappaInput = new Input<RealParameter>("kappa", "kappa parameter in TwoStructs model", Validate.REQUIRED);
    public Input<RealParameter> omegaInput = new Input<RealParameter>("omega", "kappa parameter in TwoStructs model", Validate.REQUIRED);
    ...
    public Input<RealParameter> codonProbInput_struct1 = new Input<RealParameter>("codonProbStruct1", "probabilities for each codon in Struct1", Validate.REQUIRED);    
    public Input<RealParameter> codonProbInput_struct2 = new Input<RealParameter>("codonProbStruct2", "probabilities for each codon in Struct2", Validate.REQUIRED);    
    double[] codonProb_struct1;
    double[] codonProb_struct2;
    int codonNr;
    ...
    public void initAndValidate() throws Exception {
        ...
        codonProb_struct1 = new double[codonProbInput_struct1.get().getDimension()];
        codonProb_struct2 = new double[codonProbInput_struct2.get().getDimension()];

        if (codonProb_struct1.length != codonProb_struct2.length) {
            throw new Exception("codonProbs of two structs should have equal array length");
        }
        
        codonNr = codonProb_struct1.length;
        for (int i = 0; i < codonNr; i++) {
        	codonProb_struct1[i] = codonProbInput_struct1.get().getValue(i);
        	codonProb_struct2[i] = codonProbInput_struct2.get().getValue(i);
        }
        // sanity check
        /*
    ...
    //different from OneStruct models
    double[] getCodonProb(){
    	final double percent = percentInput.get().getValue();
    	double[] codonProbResult = new double[codonNr];
    	for (int i = 0; i < codonNr; i++) {
    		codonProbResult[i] = codonProb_struct1[i] * percent + codonProb_struct2[i] * (1.0 - percent);
    	}
