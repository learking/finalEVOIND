...
public class BranchTreeLikelihood extends GenericTreeLikelihood {        
    ...
    public double calculateLogP() throws Exception {
        final BranchTree branchTree = (BranchTree) treeInput.get();
        final double[] currentBranchLengths = branchTree.getCurrentBranchLengths();

        if (traverse(branchTree.getRoot(), currentBranchLengths) != Tree.IS_CLEAN){
            calcLogP();
        }
    ...
    int traverse(final Node node, double[] currentBranchLengths) throws Exception {
        ...
        final double branchLength = currentBranchLengths[iNode];

        // First update the transition probability matrix(ices) for this branch
        //if (!node.isRoot() && (update != Tree.IS_CLEAN || branchTime != m_StoredBranchLengths[iNode])) {
        if (!node.isRoot() && (update != Tree.IS_CLEAN || branchLength != m_branchLengths[iNode])) {
            m_branchLengths[iNode] = branchLength;
            final Node parent = node.getParent();
            likelihoodCore.setNodeMatrixForUpdate(iNode);
            for (int i = 0; i < m_siteModel.getCategoryCount(); i++) {
                substitutionModel.getTransitionProbabilities(node, m_branchLengths[iNode], 0, 1.0, probabilities);
        ...
        if (!node.isLeaf()) {
            ...
            final int update1 = traverse(child1, currentBranchLengths);

            final Node child2 = node.getRight();
            final int update2 = traverse(child2, currentBranchLengths);
